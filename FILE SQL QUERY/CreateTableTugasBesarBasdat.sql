...
DROP TABLE PENGGUNA
SELECT * FROM PENGGUNA WHERE asalSekolah = 'SMP Negeri 1 Jakarta Selatan'
...
VALUES
	...
	(	'678923', '8D2E16B', '18/05/2023', 220000, 'BANK'),
...
SELECT *FROM VIDEO WHERE IdMateri = 'BIO97'
DROP TABLE VIDEO
...
--BASIC SQL QUERY
SELECT idPengguna, email FROM PENGGUNA
UNION
...
FROM TRANSAKSI
WHERE CAST(nominal AS INT) > 120000;
...
FROM VIDEO
WHERE MataPelajaran = (SELECT MataPelajaran FROM VIDEO WHERE IdMateri = 'BIO97');
...
SELECT MATERI.*, TES_AKHIR.KodeTestAkhir
FROM MATERI
FULL JOIN TES_AKHIR ON MATERI.IdMateri = TES_AKHIR.KodeTestAkhir;
...
CREATE VIEW View_Email_Surabaya AS
SELECT email
FROM PENGGUNA
WHERE asalSekolah IN (SELECT asalSekolah FROM PENGGUNA WHERE alamat LIKE '%Surabaya%');

SELECT * FROM View_Email_Surabaya

CREATE VIEW View_Transaksi_Nominal_More_Than_120000 AS
SELECT noVirtual, kodePembayaran
FROM TRANSAKSI
WHERE CAST(nominal AS INT) > 120000;

SELECT * FROM View_Transaksi_Nominal_More_Than_120000

CREATE VIEW View_Akses_Materi_TOEFL AS
SELECT DISTINCT MataPelajaran
FROM Akses_Materi
WHERE kodeTOEFl LIKE 'TOEFL%';

SELECT * FROM View_Akses_Materi_TOEFL

CREATE VIEW View_ForumDiskusi_Not_SabdaPS AS
SELECT *
FROM FORUMDISKUSI
WHERE mataPelajaran NOT IN (SELECT mataPelajaran FROM FORUMDISKUSI WHERE namaMentor = 'Sabda PS');

SELECT * FROM View_ForumDiskusi_Not_SabdaPS

CREATE VIEW View_Transaksi_After_15082022 AS
SELECT *
FROM TRANSAKSI
WHERE CONVERT(DATE, tglPembayaran, 103) > '2022-08-15';

SELECT * FROM View_Transaksi_After_15082022


CREATE VIEW View_Materi_Tes_Akhir AS
SELECT MATERI.*, TES_AKHIR.KodeTestAkhir
FROM MATERI
FULL JOIN TES_AKHIR ON MATERI.IdMateri = TES_AKHIR.KodeTestAkhir;

SELECT * FROM View_Materi_Tes_Akhir

CREATE VIEW View_Tryout_TOEFL_Cross_Joined AS
SELECT TRYOUT.kodeTryOut, TOEFL.KodeTOEFL
FROM TRYOUT
CROSS JOIN TOEFL;

SELECT * FROM View_Tryout_TOEFL_Cross_Joined

CREATE VIEW View_Akses_Materi_Joined AS
SELECT Akses_Materi.*, MATERI.Judul AS MateriJudul, MATERI.MataPelajaran AS MateriMataPelajaran
FROM Akses_Materi
INNER JOIN MATERI ON Akses_Materi.IdMateri = MATERI.IdMateri;

SELECT * FROM View_Akses_Materi_Joined

CREATE VIEW View_Akses_Materi_Left_Joined AS
SELECT Akses_Materi.*, TRYOUT.kodeTryOut AS Akses_Materi_KodeTryOut
FROM Akses_Materi
LEFT JOIN TRYOUT ON Akses_Materi.kodeTryOut = TRYOUT.kodeTryOut;

SELECT * FROM View_Akses_Materi_Left_Joined

CREATE VIEW View_Akses_Materi_Right_Joined AS
SELECT Akses_Materi.*, TOEFL.KodeTOEFL
FROM Akses_Materi
RIGHT JOIN TOEFL ON Akses_Materi.kodeTOEFl = TOEFL.KodeTOEFL;

CREATE VIEW View_Akses_Materi_Right_Joined AS
SELECT Akses_Materi.*, TOEFL.KodeTOEFL AS Akses_Materi_KodeTOEFL
FROM Akses_Materi
RIGHT JOIN TOEFL ON Akses_Materi.kodeTOEFl = TOEFL.KodeTOEFL;

SELECT * FROM View_Akses_Materi_Right_Joined
